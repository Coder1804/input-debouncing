...
function App() {
  ...
  const handleSearchedProduct = async ()=>{
    ...
    const res = await axios.get(`https://dummyjson.com/products/search?q=${inputValue}&limit=10`);
  ...
  const handleFetchOffset = async ()=>{
    const res = await axios.get(`https://dummyjson.com/products?limit=10&skip=${offsetPage * 10}`)
